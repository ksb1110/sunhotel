...
<title>키오스크 모니터링 - 부서관리 - 제주시 스마트청사안내 관리자페이지</title>
...
function changeReLoadTime3() {
	...
	//페이지 이동 URL
	var locationUrl = "/smtmng/department/monitoring/monitoringKioskList.ez?reloadTime3=" + reloadTime3 + "&responseMin=${responseMin}";
...
<div id="wrap">
	...
	<jsp:include page="/smtmng/header.ez" flush="false">
		...
		<jsp:param name="admin_menu2" value="monitoringKiosk"/>
	</jsp:include>
	
    <main id="main">
		<section class="contents-wrap">
            <h2 class="title1">키오스크 모니터링</h2>                    
            ...
            <nav class="depth3-menu">
                <ul>
                    <li <c:if test="${empty searchVO.sBuildingCode}">class="active"</c:if>>
                        <a href="<c:url value="/smtmng/department/monitoring/monitoringKioskList.ez?reloadTime3=${reloadTime3}&responseMin=${responseMin}"/>">전체건물</a>
                    </li>
                	<c:forEach var="item" items="${buildingList}">
                    <li <c:if test="${item.buildingCode == searchVO.sBuildingCode}">class="active"</c:if>>
                        <a href="<c:url value="/smtmng/department/monitoring/monitoringKioskList.ez?sBuildingCode=${item.buildingCode}&reloadTime3=${reloadTime3}&responseMin=${responseMin}"/>">${item.buildingName}</a>
                    </li>
                	</c:forEach>
                </ul>
            </nav>
		    	...
		    	<div class="table-wrapper">
		    		...
		    		<table class="table center">
						...
						<tbody>
							...
							<!-- 조회된 자료가 없을 시 -->
							<tr>
								<td colspan="8">키오스크 모니터링 정보가 없습니다.</td>
							</tr>
							</c:if>
