...
<title>등록 - 홍보이미지관리 - 직원안내도 - 부서관리 - 제주시 스마트청사안내 관리자페이지</title>

<script>
//로딩완료 후 처리되는 스크립트는 페이지 하단에 정의한다.

//등록
function insertDisplayPromotionImg() {
	...
	if(confirm("<spring:message code='confirm.ins'/>")){
		...
		$.ajax({
			...
			processData:false,
			async:false,
			url:"<c:url value='/smtmng/department/arrange/displayPromotionImgInsert.ezax'/>",
			data:parameters ,
...
<div class="win-popup">
    ...
    <form name="insert_form" id="insert_form" method="post" enctype="multipart/form-data" onsubmit="insertDisplayPromotionImg();return false;">
	
	<input type="hidden" name="facilityCode" value="${searchVO.sFacilityCode}">
	<input type="hidden" name="scrollSort" value="${nextSort}">
        ...
        <table class="table-row th-left">
            ...
            <tbody>
                <tr>
                    ...
                    <td colspan="3">
                        <input type="text" class="width100" name="imgTitle">
                    </td>
                </tr>
                <tr>
                    <th>
