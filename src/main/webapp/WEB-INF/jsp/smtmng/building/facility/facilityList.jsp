...
<title>시설정보 - 청사관리 - 제주시 스마트청사안내 관리자페이지</title>
...
function insertFacilityPop() {
	...
	window.open("${pageContext.request.contextPath}/smtmng/building/facility/facilityInsertPop.ez?sBuildingCode=${searchVO.sBuildingCode}", "facilityInsert", 'resizable=yes,scrollbars=yes,width=800,height=750');	
}

//수정 팝업
function updateFacilityPop(facilityCode) {
	
    window.open("${pageContext.request.contextPath}/smtmng/building/facility/facilityUpdatePop.ez?sFacilityCode=" + facilityCode, "facilityUpdate", 'resizable=yes,scrollbars=yes,width=800,height=780');
}

//연결직원 팝업
function listConnectEmployeePop(facilityCode) {
	
	window.open("${pageContext.request.contextPath}/smtmng/building/facility/connectEmployeeListPop.ez?sFacilityCode=" + facilityCode, "listConnectEmployeePop", 'resizable=yes,scrollbars=yes,width=800,height=750');
}
...
function deleteFacility(facilityCode) {
	...
	//키 셋팅
	$("#delete_facility_form input[name=facilityCode]").val(facilityCode);
	
	// confirm.del= 삭제하시겠습니까?
	if(confirm("<spring:message code='confirm.del'/>")) {
		
		var parameters = $("#delete_facility_form").serialize();
		
		$.ajax({
			type:"post", 
			dataType:"json",
			async:false,
			url:"<c:url value='/smtmng/building/facility/facilityDelete.ezax'/>",
			data:parameters ,
...
function fn_Search(pageIndex) {
	...
	document.frm.action = "<c:url value='/smtmng/building/facility/facilityList.ez'/>";
	document.frm.submit();
}
</script>
...
<div id="wrap">
	...
	<jsp:include page="/smtmng/header.ez" flush="false">
		...
		<jsp:param name="admin_menu2" value="facility"/>
	</jsp:include>
	
    <main id="main">
		<section class="contents-wrap">
            <h2 class="title1">시설정보</h2>     
            ...
            <nav class="depth3-menu">
                <ul>
                    <li <c:if test="${empty searchVO.sBuildingCode}">class="active"</c:if>>
                        <a href="<c:url value="/smtmng/building/facility/facilityList.ez"/>">전체건물</a>
                    </li>
                	<c:forEach var="item" items="${buildingList}">
                    <li <c:if test="${item.buildingCode == searchVO.sBuildingCode}">class="active"</c:if>>
                        <a href="<c:url value="/smtmng/building/facility/facilityList.ez?sBuildingCode=${item.buildingCode}"/>">${item.buildingName}</a>
                    </li>
                	</c:forEach>
                </ul>
            </nav>
            <!-- //3Depth Menu (메뉴없을 시 삭제) -->
           	...
           	<form name="delete_facility_form" id="delete_facility_form" style="display:none;">
           	<input type="hidden" name="facilityCode" value="">
           	</form>
           	
           	<form name="sort_facility_form" id="sort_facility_form" style="display:none;">
           	<input type="hidden" name="facilityCode" value="">
		    ...
		    <div class="item-col3-wrapper">
							...
							<tbody>
							   	<tr>
									...
									<td style="border:0;">
										...
										<button type="button" class="btn" onclick="document.location.href='<c:url value='/smtmng/building/facility/facilityList.ez?sBuildingCode=${searchVO.sBuildingCode}'/>';">전체</button>
									</td>									
							    </tr>
							</tbody>
						</table>
		    	...
		    	<div class="table-wrapper">
		    		...
		    		<table class="table center">
						...
						<tbody>
							...
							<tr>
								...
								<td <c:if test="${item.useYn eq 'N'}">style="background-color:#AFA2A2;"</c:if>>
									<c:if test="${not empty item.routeImgPath}">
									<button type="button" class="btn green sm" onclick="viewFile('${item.routeImgPath}');">보기</button>
									</c:if>
									<c:if test="${empty item.routeImgPath}">
									<img src="/images/admin/icon/temp/blue_close.png" alt="close">
									</c:if>
								</td>
								
								<td <c:if test="${item.useYn eq 'N'}">style="background-color:#AFA2A2;"</c:if>>${connectEmployeeMap.get(item.facilityCode)}</td>
								
								<td <c:if test="${item.useYn eq 'N'}">style="background-color:#AFA2A2;"</c:if>>${infoNameMap.get(item.infoType)}</td>
														
								<td <c:if test="${item.useYn eq 'N'}">style="background-color:#AFA2A2;"</c:if>>
									<button type="button" class="btn green sm" onclick="listConnectEmployeePop('${item.facilityCode}');">연결직원</button>
							...
							<!-- 조회된 자료가 없을 시 -->
							<tr>
								<td colspan="11">등록된 시설정보가 없습니다.</td>
							</tr>
							</c:if>
					...
					<div class="btn-wrap">
						<button type="button" class="btn blue" onclick="insertFacilityPop();">시설정보등록</button>
					</div>
		    	</div>
