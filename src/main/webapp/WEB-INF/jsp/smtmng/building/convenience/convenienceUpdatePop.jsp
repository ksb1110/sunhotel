...
<title>수정 - 편의시설 - 청사관리 - 제주시 스마트청사안내 관리자페이지</title>
...
function updateFacility() {
	...
	if(confirm("<spring:message code='confirm.mod'/>")){
		...
		$.ajax({
			...
			processData:false,
			async:false,
			url:"<c:url value='/smtmng/building/convenience/convenienceUpdate.ezax'/>",
			data:parameters ,
...
function deleteImg() {
	...
	if(confirm("<spring:message code='confirm.del'/>")){
		...
		$.ajax({
			...
			processData:false,
			async:false,
			url:"<c:url value='/smtmng/building/convenience/convenienceImageDelete.ezax'/>",
			data:parameters ,
			success:function(data) {
        ...
        <table class="table-row th-left">
            ...
            <tbody>
                ...
                <tr>                    
                    <th>위치정보</th>
                    <td colspan="3">
                    	LEFT : <input type="text" name="pointX" value="<c:if test="${facilityVO.pointX > 0}">${facilityVO.pointX}</c:if>" readonly>
                    	&nbsp;&nbsp;
                    	TOP : <input type="text" name="pointY" value="<c:if test="${facilityVO.pointY > 0}">${facilityVO.pointY}</c:if>" readonly>
                ...
                <tr>
                    ...
                    <td colspan="3">
                    	...
                    	<c:if test="${facilityVO.facilityImgPath == null || facilityVO.facilityImgPath == ''}">
                          	...
                          	<c:if test="${facilityVO.facilityImgPath != null && facilityVO.facilityImgPath != ''}">
                           	<div>
                           		<img src="${facilityVO.facilityImgPath}" width="100">
                           		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
