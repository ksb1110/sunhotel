...
<title>수정 - 키오스크장비 - 장비관리 - 제주시 스마트청사안내 관리자페이지</title>
...
function updateKiosk() {
	...
	if(confirm("<spring:message code='confirm.mod'/>")){
		...
		$.ajax({
			...
			processData:false,
			async:false,
			url:"<c:url value='/smtmng/equipment/kiosk/kioskUpdate.ezax'/>",
			data:parameters ,
...
<div class="win-popup">
    ...
    <form name="update_form" id="update_form" method="post" onsubmit="updateKiosk();return false;">
    
    <input type="hidden" name="kioskCode" value="${kioskVO.kioskCode}">
        ...
        <table class="table-row th-left">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" style="width:100%;" name="name" value="${kioskVO.name}">
                    </td>
             	</tr>
                <tr>
                    <th>연결시설</th>
                    <td>
                 		<select name="facilityCode">
                       		...
                       		<option value="${item.facilityCode}" <c:if test="${item.facilityCode == kioskVO.facilityCode}">selected</c:if>>${item.facilityName}</option>
                       		</c:forEach>
                       	</select>
                ...
                <tr>
                    <th>설치장소</th>
                    <td>
                        <input type="text" style="width:100%;" name="place" value="${kioskVO.place}">
                    </td>
             	</tr>
                <tr>
                    <th>참고사항</th>
                    <td>
                        <textarea name="etc" style="width:100%;height:150px;">${kioskVO.etc}</textarea>
                    </td>
                </tr>
            </tbody>
        </table>
